...
import { DogData, DogSize, ActivityLevel, DewormingFrequency } from '../types';
...
const DEWORMING_FREQ_OPTIONS: { value: DewormingFrequency; label: string; sub: string }[] = [
  { value: 'monthly', label: 'Mensal', sub: 'Ex: Milbemax, Drontal' },
  { value: 'quarterly', label: 'Trimestral', sub: 'Ex: NexGard Combo' },
...
export default function DogForm({ initialData, onSubmit }: DogFormProps) {
  ...
  const [form, setForm] = useState<DogData & { ageRange: string }>({
    ...
    lastVaccineDate: initialData?.lastVaccineDate ?? '',
    lastDewormingDate: initialData?.lastDewormingDate ?? '',
    dewormingFrequency: initialData?.dewormingFrequency ?? 'quarterly',
    lastFleaTreatmentDate: initialData?.lastFleaTreatmentDate ?? '',
    fleaTreatmentFrequency: initialData?.fleaTreatmentFrequency ?? 'monthly',
    ageRange: '',
  });
      ...
      <div className="container mx-auto px-4 max-w-lg">
        <div className="mb-6">
          ...
          <p className="text-gray-500 text-sm">
            Com base nos dados e no histórico real, vamos montar o calendário correto.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} noValidate className="space-y-5">
          ...
          <div className="card space-y-4">
            <div>
              ...
              {(errors.ageRange || errors.birthDate) && (
                <p className="text-red-500 text-xs mt-1">{errors.ageRange || errors.birthDate}</p>
              )}
            </div>
            ...
            <div className="flex gap-2">
              {ACTIVITY_OPTIONS.map((opt) => (
                ...
                >
                  ...
                  <span className="text-xs font-normal opacity-60 leading-tight hidden sm:block">{opt.sub}</span>
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="card space-y-2">
            ...
            <p className="text-xs text-gray-400">
              Deixe em branco se não souber — vamos estimar com base na idade.
            </p>
            <input
              type="date"
              value={form.lastVaccineDate}
              onChange={(e) => set('lastVaccineDate', e.target.value)}
              max={today}
              className="input-field"
            />
          </div>
          ...
          <div className="card space-y-3">
            ...
            <p className="text-xs text-gray-400">Deixe em branco se não souber.</p>
            <input
              type="date"
              value={form.lastDewormingDate}
              onChange={(e) => set('lastDewormingDate', e.target.value)}
              max={today}
              className="input-field"
            />
            <div>
              <label className="block text-xs font-semibold text-gray-600 mb-2">
                Frequência do vermífugo
              </label>
              <div className="flex gap-2">
                {DEWORMING_FREQ_OPTIONS.map((opt) => (
                  <button
                    key={opt.value}
                    type="button"
                    onClick={() => set('dewormingFrequency', opt.value)}
                    className={`toggle-btn flex-col py-2 ${
                      form.dewormingFrequency === opt.value ? 'toggle-btn-active' : 'toggle-btn-inactive'
                    }`}
                  >
                    <span className="block font-semibold text-xs">{opt.label}</span>
                    <span className="text-xs font-normal opacity-60 leading-tight hidden sm:block">{opt.sub}</span>
          ...
          <div className="card space-y-3">
            <label className="block text-sm font-semibold text-gray-700">
              🐛 Última aplicação de antipulga
            </label>
            <p className="text-xs text-gray-400">Deixe em branco se não souber.</p>
            <input
              type="date"
              value={form.lastFleaTreatmentDate}
              onChange={(e) => set('lastFleaTreatmentDate', e.target.value)}
              max={today}
              className="input-field"
            />
            <div>
              <label className="block text-xs font-semibold text-gray-600 mb-2">
                Frequência do antipulga
              </label>
              <div className="flex gap-2">
                {FLEA_FREQ_OPTIONS.map((opt) => (
                  <button
                    key={opt.value}
                    type="button"
                    onClick={() => set('fleaTreatmentFrequency', opt.value)}
                    className={`toggle-btn flex-col py-2 ${
                      form.fleaTreatmentFrequency === opt.value ? 'toggle-btn-active' : 'toggle-btn-inactive'
                    }`}
                  >
                    <span className="block font-semibold text-xs">{opt.label}</span>
                    <span className="text-xs font-normal opacity-60 leading-tight hidden sm:block">{opt.sub}</span>
